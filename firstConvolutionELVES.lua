...
--9781 to play with --- later found two unsorted elves due to bad latitude values in label. Need to remove them from dataset. 6675 6153. done later. Also ran out of memeory with a train set of 8000
trsize = 7000
tesize = 1000
...
--print(model:get(1).output:size(),model:get(1).weight:size())--,mod.output:size(),modSpatPool1.output:size(),modSpatPool1.indices:size())--modSpatConv2:size(),modTranFu
...
for i=1,nSectionsTot do
   print(i, 100*class_performance[i]/test_data.size() .. ' %')
end
gnuplot.figure()
...
gnuplot.figure()
gnuplot.imagesc(model:get(1).output[13]);
gnuplot.raw("set xrange restore")
gnuplot.raw("unset border")
gnuplot.raw("unset xtics")
gnuplot.raw("unset ytics")
...
gnuplot.figure()
gnuplot.imagesc(model:get(3).output[13]);

gnuplot.figure()
gnuplot.imagesc(model:get(4).output[13]);
gnuplot.figure()
gnuplot.imagesc(model:get(6).output[13]);

--testing another optimization library. 
--trainer = nn.StochasticGradient(model, criterion)
--grads:zero()
--trainer.learningRate = 0.001
--trainer.maxIteration = 5
--trainer:train(train_data.data)
--outputopt2 = model:forward(test_data.data[13])
--print(outputopt2)
--how is the classes output ordered?
--for i=1,outputopt2:size(1) do
--   print(i, predicted[i])
--end
