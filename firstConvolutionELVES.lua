...
for jj=1,numLoops do
   ...
   --save the data in a struct and format the train and test sets to get part of the full dataset
	 ...
	 labels = labelAllReworked[{ {1,trsize[jj]} }],
	 size = function() return trsize[jj] end
      }
      test_data = {
	 data = dataAll[{ {trsize[jj]+1,trsize[jj]+tesize},{},{},{} }],
	 labels = labelAllReworked[{ {trsize[jj]+1,trsize[jj]+tesize} }],
   ...
   ---------------------------
      ...
      reshapeInputValues = model:get(model:size()).output:size()[1] * model:get(model:size()).output:size()[2] * model:get(model:size()).output:size()[3]
	 ...
	 if i % print_every == 0 then
      ...
      --PLOT LOSSES
      if plotsPerEpoch then 
	 gnuplot.pngfigure("loss"..trsize[jj]..kk..".png")
	 gnuplot.plot({'losses',
	 ...
	 --print("The NN: \n",model)
      ...
      trerro[jj][kk] = torch.mean(torch.eq(predictions:long(), train_data.labels:long()):double())
      ...
      teerro[jj][kk] = torch.mean(torch.eq(predictions:long(), test_data.labels:long()):double())
	 ...
	 if groundtruth == indices[1] then
      ...
      if plotsPerEpoch then
	 gnuplot.pngfigure("predvstruth"..trsize[jj]..kk..".png")
	 gnuplot.plot({'Prediction',class_prediction},{'Truth',class_truth})
	 gnuplot.raw("replot")
	 gnuplot.close()
      end
      
      if plotsPerEpoch and plotsPerLayer then
         model:forward(train_data.data[18])
         --print(train_data.data[18][1])
         
         gnuplot.pngfigure("imageinput"..trsize[jj]..".png")
         --gnuplot.figure(221)
         ...
         gnuplot.pngfigure("SpatConv1"..trsize[jj]..".png")
         --gnuplot.figure(222)
         ...
         --gnuplot.figure(223)
         gnuplot.pngfigure("Trans1"..trsize[jj]..".png")
...
--   
         ...
         --gnuplot.figure(224)
         gnuplot.pngfigure("SpatMaxPool1"..trsize[jj]..".png")
      ...
      --merge 6 filtered outputs of second conv layer:
         ...
         spatToPlot = torch.cat(spatToPlot1,spatToPlot2,1):cat(spatToPlot3,1)
         --gnuplot.figure(225)
         gnuplot.pngfigure("SpatConv2"..trsize[jj]..".png")
         gnuplot.imagesc(spatToPlot)
         gnuplot.raw("unset border")
         gnuplot.raw("unset xtics")
         gnuplot.raw("unset ytics")
         ...
         transToPlot = torch.cat(transToPlot1,transToPlot2,1):cat(transToPlot3,1)
         --gnuplot.figure(226)
         gnuplot.pngfigure("Trans2"..trsize[jj]..".png")
         gnuplot.imagesc(transToPlot)
         gnuplot.raw("unset border")
         gnuplot.raw("unset xtics")
         gnuplot.raw("unset ytics")
         ...
         poolToPlot = torch.cat(poolToPlot1,poolToPlot2,1):cat(poolToPlot3,1)
         gnuplot.pngfigure("SpatMaxPool2"..trsize[jj]..".png")
         --gnuplot.figure(227)
         gnuplot.imagesc(poolToPlot)
         gnuplot.raw("unset border")
         gnuplot.raw("unset xtics")
      ...
      if plotsPerEpoch then
	 confusion = torch.Tensor(nSectionsTot,nSectionsTot):zero()
	 for i=1,test_data.size() do
	    local groundtruth = test_data.labels[i]
	    local prediction = model:forward(test_data.data[i])
	    local confidences, indices = torch.sort(prediction, true)  -- true means sort in descending order
	    confusion[groundtruth][indices[1]] = confusion[groundtruth][indices[1]] + 1
	 end
	 gnuplot.pngfigure("Confusion"..trsize[jj]..kk..".png")
	 gnuplot.imagesc(confusion:transpose(1,2),'color')
	 gnuplot.title("Confusion Matrix")
	 gnuplot.raw("set xlabel 'Prediction'")
	 gnuplot.raw("set ylabel 'Truth'")
	 gnuplot.raw("set zlabel 'Counts'")
	 gnuplot.raw('replot')
	 gnuplot.close()
      end
...
print(torch.mean(trerro,2),torch.mean(teerro,2))
print(torch.std(trerro,2),torch.std(teerro,2))
gnuplot.pngfigure("learningcurve.png")
gnuplot.plot({'train',torch.mean(trerro,2)},{'test',torch.mean(teerro,2)})
gnuplot.raw("set xlabel 'Training Set Size (x500)'")
gnuplot.raw("set ylabel 'Mean Accuracy'")
gnuplot.title("Learning Curve Mean (5 repeats)")
gnuplot.raw("replot")
gnuplot.close()
gnuplot.pngfigure("learningcurveerror.png")
gnuplot.plot({'train',torch.std(trerro,2)},{'test',torch.std(teerro,2)})
gnuplot.raw("set xlabel 'Training Set Size (x500)'")
gnuplot.raw("set ylabel 'Accuracy Std.'")
